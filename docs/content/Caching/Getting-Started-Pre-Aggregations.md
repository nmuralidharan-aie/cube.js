...
doesn't actually make sense to add up this value. Let's try doing so for
...
If we look at the source data again, let's see if a manual calculation of the
`profit_margin` matches the above. We'll use the following formula:
...
for both items sold on `2021-01-20`. For row 1:
...
Some extra considerations when for pre-aggregation selection:
...
- The query's time dimension and granularity together act as a dimension. If the
  date range isn't aligned with granularity, a common granularity is used.
...
- `rollup` pre-aggregations **always** have priority over `originalSql`. Thus,
  ...
  instruct Cube.js to use the original SQL pre-aggregations using
